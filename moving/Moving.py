...
class Moving:
    ...
    #置分子初速度与位置
    def subc2(self,no_molecule,no_cell,no_molecule_each_cell,vm_ini,cell_height,yc,p,vmean,y_length,u_wall):
        '''置分子初速度与位置'''
        pi=Constant.pi
    ...
    #计算分子运动和在上下表面的反射和取样
    def subc3(self,iloop,jnis,no_molecule,p,vmean,up_wall,wall):
        '''计算分子运动和在上下表面的反射和取样'''
        pi=Constant.pi
        dtm=Constant.dtm
        u_wall=Constant.u_wall
        vm_wall=Constant.vm_wall
        y_length=Constant.y_length


        for m in range(0,no_molecule):

            y=p[3,m]+p[1,m]*dtm  #y是模拟分子以p（1，m）运动经dtm时间后到达的位置
            if y>=y_length:  #如果y大于y_length，分子在上壁面反射
                dtr=(y-y_length)/p[1,m]  #反射后剩余的分子运动时间
                if iloop>=jnis:
                    p[0,m]=p[0,m]+u_wall/2.0
                    # **  vmean[0,m] = vmean[0,m]+u_wall/2.0
                    #这里将参考系转换为以上平板（以-u_wall/2.0运动）相联系
                    up_wall[0]=up_wall[0]+1.0
                    up_wall[1]=up_wall[1]+p[0,m]
                    # * up_wall[1] = up_wall[1]+vmean[0,m]
                    #在IP方法中u_wall[1]中累计的是入射分子的IP速度
                    up_wall[2]=up_wall[2]-p[1,m]
                    up_wall[3]=up_wall[3]+0.5*(p[0,m]**2+p[1,m]**2+p[2,m]**2)
                    #当当前循环数iloop大于开始取样循环数jnis时，进行了上平板取样（up_wall）,累计了入射分子贡献：（1）计数；（2）切向动量；（3）法向动量；（4）动能。
                abc1=math.sqrt(-math.log(rd.random()))*vm_wall
                abc2=2.0*pi*rd.random()
                p[0,m]=abc1*math.sin(abc2)
                p[2,m]=abc1*math.cos(abc2)
                p[1,m]=-math.sqrt(-math.log(rd.random()))*vm_wall
                # ** vmean[0,m] = 0.0
                # ** vmean[1,m] = 0.0
                # ** vmean[2,m] = 0.0
                #计算了在上平板漫反射后的分子速度，反射后的IP速度在与平板相联系的坐标中为零
                if iloop>=jnis:
                    up_wall[4]=up_wall[4]+p[0,m]
                    # * up_wall[4] = up_wall[4] + vmean[0, m]
                    #在IP方法中up_wall(4)中累计的是反射分子的IP速度
                    up_wall[5]=up_wall[5]+p[1,m]
                    up_wall[6]=up_wall[6]+0.5*(p[0,m]**2+p[1,m]**2+p[2,m]**2)
                #以上累计了反射分子贡献：（4）切向动量(为0)；（5）法向动量；（6）动能。
                p[0,m]=p[0,m]-u_wall/2.0
                # **  vmean[0,m] = -u_wall/2.0
                # ** vmean[1, m] = 0.0
                p[3,m]=y_length+p[1,m]*dtr
    ...
    def subc4(self,no_molecule,no_cell,cell_height,p,vmean,ic,lcr):
        '''原始号码为M的分子根据其运动后所在网格重新编号为K.M存于LCR(K)中。'''

        for icell in range(0,no_cell):
            ic[0,icell]=0
        for m in range(0,no_molecule):
            ncell=int(p[3,m]/cell_height)
            # if ncell==0 :
            #     ncell = 1
            if ncell>=no_cell:
        ...
        for m in range(0,no_molecule):
            ...
            if ncell>=no_cell:
                ncell=no_cell-1
            ic[0,ncell]=ic[0,ncell]+1
            k=int(ic[1,ncell]+ic[0,ncell])
            lcr[k]=m
            #此循环又数出了IC[0,I]，并得到了原始号码为M的分子根据其新位置的编号K。将M置于LCR[K]中。

    #计算碰撞
    def subc5(self,no_cell,no_molecule,coll_remain,ic,lcr,p,vmean):
        '''计算碰撞'''
        vrc=[0.0]*3
        vccm=[0.0]*3
        pi=Constant.pi
        ...
        for m in range(0,no_cell):
            ...
            for icoll in range(0,icontr):
                k=int(rd.random()*ic[0,m]+ic[1,m]+0.9999999)
                if k==ic[1,m]:
                    k=k+1
                l1=lcr[k]  #在M网格在随机选中一个分子l1
                j=int(rd.random()*ic[0,m]+ic[1,m]+0.9999999)
                if j==ic[1,m]:
                    j=j+1
                while (j==k):
                    j=int(rd.random()*ic[0,m]+ic[1,m]+0.9999999)
            ...
            while True:
                ...
                for k in range(0,3):
                    ...
                    # vccm为分子对质心速度分量，这里得到了l1，l2分子碰撞后的速度
                    # ** ve_macro = 0.5*(vmean[k,l1]+vmean[k,l2])
                    # ** vmean[k,l1] = ve_macro
                    # ** vmean[k,l2] = ve_macro
                    # 动量守恒并均分给两个分子，得到碰撞后的信息速度
    ...
    #流场参数求和
    def subc6(self,no_cell,no_molecule,ic,lcr,p,vmean,field):
        '''流场参数求和'''
    ...
    #输出相关参数
    def subc7(self,nloop,jnis,no_cell,yc,no_molecule_each_cell,wall,up_wall,field,op):
        ...
        f_up_wall=open("E:\\pythonProject\\Moving\\up_wall_data.csv","a",newline='')
        csv_writer1=csv.writer(f_up_wall,dialect="excel")
        ...
        a=(nloop-jnis)*dtm*(fnd*area*vm_ini)
        op[1]=up_wall[0]/a
        op[2]=up_wall[2]/(vm_ini*a)
        op[3]=up_wall[5]/(vm_ini*a)
        op[4]=2.0*math.sqrt(pi)*up_wall[1]/(u_wall*a)
        op[5]=2.0*math.sqrt(pi)*up_wall[4]/(u_wall*a)
        op[6]=up_wall[3]/(a*vm_ini**2)
        op[7]=up_wall[6]/(a*vm_ini**2)
        drag=rmass*up_wall[1]/((nloop-jnis)*dtm*area)
        f_down_wall=open("E:\\pythonProject\\Moving\\down_wall_data.csv","a",newline='')  #加newline后写入文件内容无空行
        csv_writer2=csv.writer(f_down_wall,dialect="excel")
        csv_writer2.writerow(op)
        f_down_wall.close()
        ...
        for n in range(0,no_cell):
            ...
            f_field=open("E:\\pythonProject\\Moving\\field.csv","a",newline='')
