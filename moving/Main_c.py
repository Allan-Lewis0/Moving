...
jnis=50 #开始取样循环数
nloop=500 #总循环数
p=np.array([[0.0]*no_molecule for i in range(4)]) # p[0],p[1],p[2]为分子速度在x,y,z方向上的分量,p[3]为分子在y方向上的位置
vmean=np.array([[0.0]*no_molecule for i in range(3)]) #vmean[i,n],(0)(1)(2)为分子的信息速度在x,y,z方向上的分量
lcr=[0]*no_molecule*2 #lcr[n]中存放新位置编码为n的分子的原始号码
...
up_wall=[0.0]*7 #up_wall[i]是在上平板累计的以上各量
...
for n in range(0,no_cell):
    yc[n] = (n+0.5)*cell_height #第N网格中心的坐标
...
print("模拟分子运中......")
#置分子初速度与位置
moving.subc2(no_molecule,no_cell,no_molecule_each_cell,vm_ini,cell_height,yc,p,vmean,y_length,u_wall)
while(iloop<nloop): #当前循环数小于总循环数时，继续循环
    ...
    for m in range(0,no_molecule):
        ...
        if iloop==1:
            ...
            next_x=x_values[-no_molecule]+p[0,m]*dtm
            next_y=p[3,m]
            next_z=z_values[-no_molecule]+p[2,m]*dtm
            x_values.append(next_x)
            y_values.append(next_y)
            z_values.append(next_z)

    # next_x1=x_values[(iloop-1)*no_molecule]
    # next_y1=y_values[(iloop-1)*no_molecule]
    # next_z1=z_values[(iloop-1)*no_molecule]
    ...
    moving.subc3(iloop,jnis,no_molecule,p,vmean,up_wall,wall)
    #分子排序，编号
    moving.subc4(no_molecule,no_cell,cell_height,p,vmean,ic,lcr)
    #计算碰撞
    moving.subc5(no_cell,no_molecule,coll_remain,ic,lcr,p,vmean)
    if iloop>jnis: #当前循环数大于开始取样循环数时进行壁面和流场取样
        moving.subc6(no_cell,no_molecule,ic,lcr,p,vmean,field)
#当前循环数等于总循环数时进行取样和输出
moving.subc7(nloop,jnis,no_cell,yc,no_molecule_each_cell,wall,up_wall,field,op)
print("{}个分子进行了{}个时间步长的运动，总共生成{}个点".format(no_molecule,iloop,len(x_values)-1))
# print(len(x_values_std))

#绘制点的运动范围
ax = plt.subplot(111, projection='3d')  # 创建一个三维的绘图工程
# 将数据点分成三部分画，在颜色上有区分度
ax.scatter(x_values, y_values, z_values, s=2,c='y')  # 绘制数据点
